...
def is_valid_choice(cand, word, meaning, hint):
    cand_clean = re.sub(r'[“”"《》【】]', '', cand)
    w_clean = re.sub(r'[“”"《》【】]', '', word)
    if cand == word or cand_clean == w_clean:
        return False
    # Distractor cannot appear in the remaining hint text
    if cand_clean in hint.replace("______", ""):
        return False
    return True

def generate_strictly_filtered_distractors(word, chapter, group, meaning, hint):
    w = word.strip()
    w_clean = re.sub(r'[“”"《》【】]', '', w)
    
    # 1. Exact map
    if w in EXACT_KEYWORDS_MAP:
        candidates = EXACT_KEYWORDS_MAP[w]
        valid = [c for c in candidates if is_valid_choice(c, w, meaning, hint)]
        if len(valid) >= 3:
            return valid[:3]
            
    if w_clean in EXACT_KEYWORDS_MAP:
        candidates = EXACT_KEYWORDS_MAP[w_clean]
        valid = [c for c in candidates if is_valid_choice(c, w, meaning, hint)]
        if len(valid) >= 3:
            return valid[:3]
            
    # 2. Check domain pools
    for pool_name, pool_words in DOMAIN_POOLS.items():
        if w in pool_words or w_clean in [re.sub(r'[“”"《》【】]', '', x) for x in pool_words]:
            valid = [x for x in pool_words if is_valid_choice(x, w, meaning, hint)]
            if len(valid) >= 3:
                return valid[:3]
                
    # 3. Structural fallback based on length
    L = len(w_clean)
    if L == 4:
        pool = DOMAIN_POOLS["strategic_position"] + DOMAIN_POOLS["work_principles"]
        valid = [x for x in pool if is_valid_choice(x, w, meaning, hint)]
        return valid[:3]
    elif L == 2:
        ...
        valid = [x for x in DOMAIN_POOLS["governance_3"] if is_valid_choice(x, w, meaning, hint)]
        return valid[:3]
    elif 5 <= L <= 6:
        valid = [x for x in DOMAIN_POOLS["core_number_terms"] if is_valid_choice(x, w, meaning, hint)]
        return valid[:3]
    else:
        pool = [
...
processed = 0
for item in data:
    d_list = generate_strictly_filtered_distractors(
        item['word'], item.get('chapter', ''), item.get('group', ''), item.get('meaning', ''), item.get('hint', '')
    )
    item['distractors'] = [{"word": dw, "meaning": dw, "hint": dw} for dw in d_list[:3]]
    processed += 1

print(f"Enforced strict stem-exclusion and domain filtering for {processed} items.")
